...
if (document.URL.match(/index/)) {
  ...
  document.querySelector(
    ".mainvisual_catch"
  ).style.height = `calc(${vh}px - 150px)`;

  document.addEventListener("DOMContentLoaded", function () {
    gsap.set(".preview_box", { width: 0 });

    const images = [
      "/images/img_work01.png",
      "/images/img_work02.png",
      "/images/img_work03.png",
      "/images/img_work04.png",
      "/images/img_work05.png",
      "/images/img_work06.png",
    ];

    //loader

    const tlLoder = gsap.timeline({ defaults: { ease: "click.out" } });

    tlLoder.to(".text", { y: "0%", duration: 1, stagger: 0.25 });
    tlLoder.to(".slider", { y: "-100%", duration: 1.5, delay: 0.5 });
    tlLoder.to(".intro", { y: "-100%", duration: 1 }, "-=1");
    tlLoder.fromTo("header", { opacity: 0 }, { opacity: 1, duration: 1 });
    tlLoder.fromTo(
      ".mainvisual_catch",
      { opacity: 0 },
